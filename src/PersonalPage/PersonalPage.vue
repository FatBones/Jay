    ...
    <div class="items">
      <ul class="song-group">
        <li
          ...
          :key="index"
          @click.once="clickFun(index)"
        >
          {{ i.name }}
          <audio
            class="audio"
            ref="audio"
            :src="require('../assets/audio' + information[index].songSrc)"
            @ended="endFun()"
...
export default {
  ...
  beforeCreate() {
    document.body.setAttribute(
      "style",
      "background-image: linear-gradient(to right, #578c94, #559398, #52999c, #51a09f, #50a6a1);"
    );
  },
  beforeMount() {
    this.getSong();
  ...
  mounted() {
    ...
    this.$bus.$emit("isInPersonalPage", true);
  },
  beforeDestroy() {
    this.$bus.$emit("isInPersonalPage", false)
